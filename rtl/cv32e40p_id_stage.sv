...
//     output logic ctrl_busy_o,       // to sleep unit
//     output logic is_decoding_o,   // to EX stage, not sure wether flop
...
//     input  logic data_misaligned_i,   // from LSU
...
//     output logic        irq_ack_o,    //output of the CORE, not sure wether flop
//     output logic [ 4:0] irq_id_o,     //output of the CORE, not sure wether flop
//     output logic [ 4:0] exc_cause_o,  // some logic operations in CORE and the result to IF stage,
...
#(
    ...
    // Jumps and branches
    output logic        branch_in_ex_o,
    input  logic        branch_decision_i,   //from EX stage, if the branch is taken, assign branch_decision_o = alu_cmp_result;
    output logic [31:0] jump_target_o,
    output logic [ 1:0] ctrl_transfer_insn_in_dec_o,
    ...
    input logic is_fetch_failed_i,  //to conroller, if the fetch failed, the controller will not send a new request to IF stage
    input logic [31:0] pc_id_i,   // Program Counter from IF stage, used for jump target calculation
    ...
    output logic [5:0] regfile_waddr_ex_o,    // Register file write address
    output logic       regfile_we_ex_o,

    output logic [5:0] regfile_alu_waddr_ex_o,  // Register file write address for ALU result
    output logic       regfile_alu_we_ex_o,

    // ALU
    output logic              alu_en_ex_o,      // ALU is enabled
    output alu_opcode_e       alu_operator_ex_o,    // ALU operation selection
    output logic              alu_is_clpx_ex_o,   // ALU is a CLPX operation  
    output logic              alu_is_subrot_ex_o,   // ALU is a subrotation operation
    output logic        [1:0] alu_clpx_shift_ex_o,   // CLPX shift amount 
    ...
    // APU
    output logic                              apu_en_ex_o,      // APU is enabled
    output logic [     APU_WOP_CPU-1:0]       apu_op_ex_o,    // APU operation selection
    output logic [                 1:0]       apu_lat_ex_o,   // APU latency
    output logic [   APU_NARGS_CPU-1:0][31:0] apu_operands_ex_o,    // APU operands
    output logic [APU_NDSFLAGS_CPU-1:0]       apu_flags_ex_o,     // APU flags
    output logic [                 5:0]       apu_waddr_ex_o,    // APU write address

    output logic [2:0][5:0] apu_read_regs_o,      // APU read registers
    output logic [2:0]      apu_read_regs_valid_o,      // APU read registers valid
    input  logic            apu_read_dep_i,     // from EX stafe, APU read dependency, if the APU is reading a register that is written in the current instruction
    input  logic            apu_read_dep_for_jalr_i,    // from EX stage, APU read dependency for JALR, if the APU is reading a register that is written in the current instruction and the instruction is a JALR
    output logic [1:0][5:0] apu_write_regs_o,     // APU write registers
    output logic [1:0]      apu_write_regs_valid_o,   // APU write registers valid
    input  logic            apu_write_dep_i,  // from EX stage, APU write dependency, if the APU is writing a register that is read in the current instruction
    output logic            apu_perf_dep_o,   //apu stall, from controller
    input  logic            apu_busy_i,     //from EX stage, APU is busy, if the APU is busy and cannot accept new requests

    input logic            fs_off_i,  // from CSR, FPU is off, if the FPU is off, the APU cannot be used
    input logic [C_RM-1:0] frm_i,       // from CSR, FPU rounding mode

    // CSR ID/EX
    output logic              csr_access_ex_o, // CSR access
    output csr_opcode_e       csr_op_ex_o,    // CSR operation selection
    input  PrivLvl_t          current_priv_lvl_i, // Current privilege level
    output logic              csr_irq_sec_o,    // CSR IRQ security
    output logic        [5:0] csr_cause_o,  
    output logic              csr_save_if_o,
    output logic              csr_save_id_o,
    output logic              csr_save_ex_o,
  ...
  assign imm_iz_type = {20'b0, instr[31:20]};
  assign imm_s_type = {{20{instr[31]}}, instr[31:25], instr[11:7]};
  assign imm_sb_type = {{19{instr[31]}}, instr[31], instr[7], instr[30:25], instr[11:8], 1'b0};  //末位补零实现2/4字节对齐
  assign imm_u_type = {instr[31:12], 12'b0};
  assign imm_uj_type = {{12{instr[31]}}, instr[19:12], instr[20], instr[30:21], 1'b0};
  ...
  always_ff @(posedge clk, negedge rst_n) begin : DQ
    if (rst_n == 1'b0) begin
        regfile_we_ex_o_q <= '0;
        regfile_waddr_ex_o_q <= '0;
        branch_in_ex_o_q <= '0;
        apu_en_ex_o_q <= '0;
        apu_lat_ex_o_q <= '0;
        data_req_ex_o_q          <= '0;
